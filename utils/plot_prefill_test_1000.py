#!/usr/bin/env python3
# utils/plot_prefill_test_1000.py
#
# Plots results from the prefill_test_1000 experiment.
# Generates three figures: throughput, latency, and goodput — each with one
# subplot per request rate and one line per ratio (colocated vs 1p1d).
#
# Usage:
#   python utils/plot_prefill_test_1000.py
#   python utils/plot_prefill_test_1000.py --results-dir /path/to/stats --plots-dir /path/to/plots
...
parser.add_argument("--results-dir", default=str(_repo_root / "results" / "prefill_test_1000"))
parser.add_argument("--plots-dir",   default=str(_repo_root / "results" / "prefill_test_1000" / "plots"))
...
def plot_latency(records):
    ...
    fig.suptitle("Latency vs Input Length\nColocated vs 1p1d Disaggregated", y=1.01)
    plt.tight_layout()
    out = PLOTS_DIR / "prefill_test_1000_latency.png"
    plt.savefig(out, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {out.name}")
