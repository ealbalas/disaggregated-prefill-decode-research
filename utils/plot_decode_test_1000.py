#!/usr/bin/env python3
# utils/plot_decode_test_100.py
#
# Plots results from the decode_test_100 experiment.
# Generates three figures: throughput, latency, and goodput — each with one
# subplot per request rate and one line per ratio (colocated vs disaggregated).
#
# Usage:
#   python utils/plot_decode_test_100.py
#   python utils/plot_decode_test_100.py --results-dir /path/to/stats --plots-dir /path/to/plots
...
def _subplots_per_rate(records, metric_key, ylabel, title, filename):
    """One subplot per request rate, x=output_len, one line per ratio."""
...
def plot_latency(records):
    """TTFT, TPOT, E2EL vs output length — 3-row × n-rate subplot grid."""
    rates = sorted({r["rate"] for r in records})
    if not rates:
        return
    ...
    fig.suptitle("Latency vs Output Length\nColocated vs Disaggregated", y=1.01)
    plt.tight_layout()
    out = PLOTS_DIR / "decode_test_100_latency.png"
    plt.savefig(out, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {out.name}")
...
def plot_goodput(records):
    """Goodput (req/s meeting all SLOs) vs output length."""
