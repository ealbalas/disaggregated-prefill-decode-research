#!/usr/bin/env python3
# utils/plot_simple_compare.py
#
# Plots results from the simple colocated vs disaggregated comparison experiment.
# Generates three figures: throughput, latency, and goodput — each with one
# subplot per request rate and one line per ratio.
#
# Usage:
#   python utils/plot_simple_compare.py
#   python utils/plot_simple_compare.py --results-dir /path/to/stats --plots-dir /path/to/plots
...
RATIOS = ["colocated", "1p1d", "2p1d", "3p1d"]
COLORS = {r: cm.tab10(i / (len(RATIOS) - 1)) for i, r in enumerate(RATIOS)}
...
def plot_throughput(records):
    ...
    _subplots_per_rate(
        ...
        "simple_compare_output_throughput.png",
    )
...
def plot_latency(records):
    ...
    plt.tight_layout()
    out = PLOTS_DIR / "simple_compare_latency.png"
    plt.savefig(out, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {out.name}")
