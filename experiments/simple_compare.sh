#!/usr/bin/env bash
# experiments/simple_compare.sh
#
# Simple colocated vs disaggregated comparison.
# Sweeps input length (256 → 8096, 4 points) with output fixed at 256.
# Uses a single concurrency (16) and two request rates (2, 10).
# Tests colocated baseline plus 1p1d, 2p1d, 3p1d disaggregated ratios.
#
# Usage:
#   bash experiments/simple_compare.sh
#   bash experiments/simple_compare.sh configs/simple_compare.env
...
SWEEP_CONFIG="${1:-$REPO_ROOT/configs/simple_compare.env}"
source "$SWEEP_CONFIG"
...
echo "Starting simple comparison: $TOTAL total runs."
...
echo ""
echo "Simple comparison complete. Results in $OUTDIR"
echo "Run: python utils/plot_simple_compare.py to generate plots."
