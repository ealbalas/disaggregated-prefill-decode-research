#!/usr/bin/env bash
# experiments/prefill_stress.sh
#
# Prefill stress experiment: colocated vs disaggregated under heavy prefill load.
# Results saved to $OUTDIR, plots to $OUTDIR/plots.
#
# Usage:
#   bash experiments/prefill_stress.sh
#   bash experiments/prefill_stress.sh configs/prefill_stress.env
...
SWEEP_CONFIG="${1:-$REPO_ROOT/configs/prefill_stress.env}"
source "$SWEEP_CONFIG"
...
echo ""
echo "Prefill stress experiment complete. Results in $OUTDIR"
echo "Run: python utils/plot_prefill_stress.py to generate plots."
