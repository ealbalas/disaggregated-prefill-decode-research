#!/usr/bin/env bash
# experiments/ratio_characterization.sh
#
# Ratio characterization: compares all 5 P:D ratios (1p1d, 1p2d, 1p3d, 2p1d,
# 3p1d) plus a colocated baseline across three workload archetypes:
#   - prefill-heavy  (1024x64)
#   - balanced       (256x256)
#   - decode-heavy   (64x1024)
#
# INPUT_LENS and OUTPUT_LENS are index-paired, not crossed.
# Results saved to $OUTDIR with tags: {ratio}_{input}x{output}_rate{rate}_conc{concurrency}
#
# Usage:
#   bash experiments/ratio_characterization.sh
#   bash experiments/ratio_characterization.sh configs/ratio_characterization.env
...
OUTDIR="$SCRATCH/disaggregated-prefill-decode-research/results/ratio_characterization"

SWEEP_CONFIG="${1:-$REPO_ROOT/configs/ratio_characterization.env}"
source "$SWEEP_CONFIG"
...
echo "Starting ratio characterization: $TOTAL total runs."
...
echo ""
echo "Ratio characterization complete. Results in $OUTDIR"
echo "Run: python utils/plot_ratio_characterization.py to generate plots."
