#!/usr/bin/env bash
# experiments/kv_transfer_isolation.sh
#
# KV transfer isolation: measures Nixl KV transfer overhead as a function of
# input length. With output_len=1, TTFT ≈ prefill_time + transfer_time.
# The transfer overhead is derived in the plot script as:
#   transfer_overhead[input_len] = mean_ttft_1p1d - mean_ttft_colocated
#
# Results saved to results/kv_transfer_isolation/
#
# Usage:
#   bash experiments/kv_transfer_isolation.sh
#   bash experiments/kv_transfer_isolation.sh configs/kv_transfer_isolation.env
...
OUTDIR="$SCRATCH/disaggregated-prefill-decode-research/results/kv_transfer_isolation"

SWEEP_CONFIG="${1:-$REPO_ROOT/configs/kv_transfer_isolation.env}"
source "$SWEEP_CONFIG"
...
echo "Starting KV transfer isolation: $TOTAL total runs."
echo "  Input lengths: ${INPUT_LENS[*]} (all with output_len=1)"
...
echo ""
echo "KV transfer isolation complete. Results in $OUTDIR"
echo "Run: python utils/plot_kv_transfer_isolation.py to generate plots."
