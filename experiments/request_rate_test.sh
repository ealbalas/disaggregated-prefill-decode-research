#!/usr/bin/env bash
# experiments/request_rate_test.sh
#
# Request rate sweep: colocated vs disaggregated ratios across request rates
# (2, 4, 8, 10 req/s). Fixed at 1024-token input, 256-token output, concurrency 32.
...
#   bash experiments/request_rate_test.sh configs/request_rate_test.env
...
OUTDIR="$SCRATCH/disaggregated-prefill-decode-research/results/request_rate_test"

SWEEP_CONFIG="${1:-$REPO_ROOT/configs/request_rate_test.env}"
source "$SWEEP_CONFIG"
...
echo "Starting request rate test: $TOTAL total runs."
...
echo ""
echo "Request rate test complete. Results in $OUTDIR"
echo "Run: python utils/plot_request_rate_test.py to generate plots."
